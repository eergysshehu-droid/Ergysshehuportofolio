...
test('homepage mobile photo stacking stays enabled', async ({page}, testInfo) => {
  ...
  const state = await card.evaluate(element => {
    const section = element.closest('.work-section');
    const grid = element.closest('.project-grid');

    return {
      cardPosition: getComputedStyle(element).position,
      cardTop: getComputedStyle(element).top,
      sectionOverflow: section ? getComputedStyle(section).overflow : null,
      gridOverflow: grid ? getComputedStyle(grid).overflow : null
    };
  });

  expect(state.cardPosition).toBe('sticky');
  expect(state.cardTop).toBe('0px');
  expect(state.sectionOverflow).not.toBe('hidden');
...
test('Book form submits through API enhancement', async ({page}) => {
  ...
  expect(payload.name).toBe('E2E Test');
