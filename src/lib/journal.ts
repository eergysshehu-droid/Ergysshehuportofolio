...
export function journal(): Promise<Post[]> {
  ...
  return cached ??= (client ? client.fetch(`*[_type == "post" && defined(slug.current)] | order(publishedAt desc) {
    title,
    titleSq,
    "slug": slug.current,
    excerpt,
    ...
    .then((posts: Post[]) => [...new Map([...(localJournal as Post[]), ...posts].map(p => [p.slug, p])).values()]
