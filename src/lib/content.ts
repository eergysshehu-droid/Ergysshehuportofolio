...
import instagramPortfolio from '../data/instagram-portfolio.json';
...
export interface Album {title: string; slug: string; description?: string; category?: string; kind?: string; featured?: boolean; videoUrl?: string; localVideo?: string; instagramUrl?: string; credits?: string; cover?: Photo; photos?: Photo[]; location?: string; year?: number}
export interface Settings {name?: string; headline?: string; intro?: string; aboutHeading?: string; bio?: string; biographyEn?: string; biographySq?: string; artistQuote?: string; portrait?: Photo; heroImage?: Photo; fashionCover?: Photo; weddingsCover?: Photo; portraitsCover?: Photo; filmCover?: Photo; reelUrl?: string; email?: string; instagram?: string; facebook?: string; youtube?: string; linkedin?: string; seoDescription?: string}
...
const baseline={settings:localPortfolio.settings,albums:[...localPortfolio.albums,...instagramPortfolio.albums]};
export function content(): Promise<{settings: Settings; albums: Album[]}> {
  ...
  }`).then(data => ({settings: {...baseline.settings, ...Object.fromEntries(Object.entries(data.settings || {}).filter(([,v]) => v != null))}, albums: [...new Map([...baseline.albums, ...(data.albums || [])].map(a => [a.slug,a])).values()]})) : Promise.resolve(baseline));
}
